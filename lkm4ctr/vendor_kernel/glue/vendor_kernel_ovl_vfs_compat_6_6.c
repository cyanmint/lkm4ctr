...
/*
 * vendor_kernel_ovl_vfs_compat_6_6.c - resolves the VFS-internal helpers
 * declared in vendor_kernel_ovl_vfs_compat_6_6.h at module load time. This is
 * NEW code (not vendored from kernel-common). See
 * vendor_kernel_ovl_vfs_compat_6_6.h for the full rationale.
 */
 ...
 * vendor_kernel_ovl_vfs_compat_6_6.h (must be built from this same file,
 * before that header's #define pass has any effect).
 */
#undef prepare_creds
#undef errseq_sample
#undef mntput
...
#undef VNS_OVL_VFSC_6_6_DEFINE

int vns_ovl_vfs_compat_6_6_resolve(void)
{
#define VNS_OVL_VFSC_6_6_RESOLVE(name) \
	do { \
		vns_ovl_vfsc_6_6_##name = (typeof(vns_ovl_vfsc_6_6_##name)) \
			shadow_hook_resolve(#name); \
		if (!vns_ovl_vfsc_6_6_##name) { \
			LKM4CTR_ERR("vendor_kernel", \
				    "overlayfs_6_6: could not resolve VFS symbol '%s'; vendored overlayfs unavailable", \
				    #name); \
			return -ENOENT; \
		} \
	} while (0);
...
#endif /* LINUX_VERSION_CODE in [6.6, 6.7) */
