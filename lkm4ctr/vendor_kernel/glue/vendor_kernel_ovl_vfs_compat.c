...
/*
 * vendor_kernel_ovl_vfs_compat.c - resolves the VFS-internal helpers declared in
 * vendor_kernel_ovl_vfs_compat.h at module load time. This is NEW code (not vendored
 * from kernel-common). See vendor_kernel_ovl_vfs_compat.h for the full rationale.
 */
 ...
 * vendor_kernel_ovl_vfs_compat.h (must be built from this same file, before that header's
 * #define pass has any effect - so include it only once, after these).
 */
#undef mount_nodev
#undef prepare_creds
#undef errseq_sample
