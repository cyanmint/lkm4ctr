...
/*
 * vendor_kernel_ovl_vfs_compat_5_15.c - resolves the VFS-internal helpers
 * declared in vendor_kernel_ovl_vfs_compat_5_15.h at module load time. This
 * is NEW code (not vendored from kernel-common). See
 * vendor_kernel_ovl_vfs_compat_5_15.h for the full rationale.
 */
 ...
 * vendor_kernel_ovl_vfs_compat_5_15.h (must be built from this same file,
...
#undef VNS_OVL_VFSC_5_15_DEFINE

int vns_ovl_vfs_compat_5_15_resolve(void)
{
#define VNS_OVL_VFSC_5_15_RESOLVE(name) \
	do { \
		vns_ovl_vfsc_5_15_##name = \
			(typeof(vns_ovl_vfsc_5_15_##name))shadow_hook_resolve(#name); \
		if (!vns_ovl_vfsc_5_15_##name) { \
			LKM4CTR_ERR("vendor_kernel", \
				    "overlayfs_5_15: could not resolve VFS symbol '%s'; vendored overlayfs unavailable", \
				    #name); \
			return -ENOENT; \
		} \
	} while (0);
	VNS_OVL_VFS_COMPAT_5_15_LIST(VNS_OVL_VFSC_5_15_RESOLVE)
#undef VNS_OVL_VFSC_5_15_RESOLVE
	return 0;
}

#endif /* LINUX_VERSION_CODE in [5.15, 6.1) */
