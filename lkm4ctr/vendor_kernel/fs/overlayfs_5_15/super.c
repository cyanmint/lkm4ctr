...
/*
 * lkm4ctr [BUILD-COMPAT]: this vendored overlayfs source was taken from a kernel in the
 * [5.15, 6.1) VFS API era (legacy mount(get_sb)-style
 * file_system_type using ovl_mount()/mount_nodev(), no idmap
 * parameters on any vfs_*() call, no struct mnt_idmap or struct
 * user_namespace idmap arguments at all, no fs_context support).
 * Taken from android14-5.15 (android13-5.15 is nearly identical;
 * android14-5.15 carries a small ovl_i_path_real() cleanup fix
 * that applies equally to both, since neither is distinguishable
 * by LINUX_VERSION_CODE).
...
/*
 * [BUILD-COMPAT] vendored into lkm4ctr.ko: this file's file_system_type is
 * never register_filesystem()'d directly (see vns_ovl_fs_type below) -- it
 * is instead always handed out by a hook on get_fs_type()
 * (glue/vendor_kernel_overlay.c), unconditionally, for the name "overlay",
 * so there is nothing to unregister/collide with. Uses the same name,
 * "overlay", as upstream. vns_ovl_mount_count is new (not upstream) and is
 * surfaced through vendor_kernel's diagfs status (see
 * glue/vendor_kernel_diag.c).
 */
atomic_t vns_ovl_mount_count = ATOMIC_INIT(0);
 ...
 * (see glue/vendor_kernel_ovl_vfs_compat_5_15.h) -- its entire body is
...
static int ovl_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	atomic_inc(&vns_ovl_mount_count); /* [BUILD-COMPAT] diagfs stat, see above */
	return 0;
...
static void vns_ovl_kill_sb(struct super_block *sb)
{
	atomic_dec(&vns_ovl_mount_count); /* [BUILD-COMPAT] diagfs stat, see above */
	kill_anon_super(sb);
}

/*
 * [BUILD-COMPAT] non-static (was static "ovl_fs_type" on kernels where it
 * was static in the first place): this vendored copy is deliberately
 * never passed to register_filesystem()/unregister_filesystem() -- doing
 * so with the same name "overlay" the running kernel's own overlayfs
 * (built-in or a separate overlay.ko) may already be using would either
 * collide with -EBUSY, or -- if it "won" the race -- shadow the real one
 * for every caller, not just the ones this module intends to help.
 * Instead, glue/vendor_kernel_overlay.c hooks get_fs_type() and always
 * hands out this struct for the name "overlay", regardless of what the
 * running kernel's own overlayfs support looks like, so
 * "mount -t overlay ..." always resolves to this vendored implementation
 * while lkm4ctr.ko is loaded.
 */
struct file_system_type vns_ovl_fs_type = {
	.owner		= THIS_MODULE,
	.name		= "overlay",
	.fs_flags	= FS_USERNS_MOUNT,
	.mount		= ovl_mount,
	.kill_sb	= vns_ovl_kill_sb,
};
...
int vns_ovl_init(void)
{
	 ...
	 * running kernel (see glue/vendor_kernel_ovl_vfs_compat_5_15.h).
	 * Must happen before anything below (or any later ovl_mount()/file
	 * operation) can call through to them.
	 */
	err = vns_ovl_vfs_compat_5_15_resolve();
	if (err)
		return err;
	...
	/* [BUILD-COMPAT] no register_filesystem() call here, see vns_ovl_fs_type above */
	return 0;
}

void vns_ovl_exit(void)
{
	/* [BUILD-COMPAT] no unregister_filesystem() call here, see vns_ovl_fs_type above */
...
/*
 * [BUILD-COMPAT] not module_init()/module_exit(): lkm4ctr.ko already has a
 * single module_init/module_exit pair in lkm4ctr_main.c. vns_ovl_init()/
 * vns_ovl_exit() are instead called from vendor_kernel_init()/
 * vendor_kernel_exit() (glue/vendor_kernel_overlay.c).
 */

#endif /* LINUX_VERSION_CODE in [KERNEL_VERSION(5, 15, 0), KERNEL_VERSION(6, 1, 0)) */
