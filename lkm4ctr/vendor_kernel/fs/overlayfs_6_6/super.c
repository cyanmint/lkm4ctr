...
/*
 * lkm4ctr [BUILD-COMPAT]: this vendored overlayfs source was taken from android15-6.6
 * (kernel 6.6.x), a later VFS/fs_context API era than 6.1: idmap
 * arguments use struct mnt_idmap * (not struct user_namespace *),
 * mnt_idmap()/mnt_user_ns() replaces the 6.1-era mnt_user_ns()
 * usage, and the file_system_type uses .init_fs_context
 * (ovl_init_fs_context()/fs_context_operations, params.c) instead
 * of the legacy .mount callback.
...
#endif /* LINUX_VERSION_CODE in [KERNEL_VERSION(6, 6, 0), KERNEL_VERSION(6, 7, 0)) */
