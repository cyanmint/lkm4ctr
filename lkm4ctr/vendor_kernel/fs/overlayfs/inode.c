...
struct posix_acl *ovl_get_acl(struct inode *inode, int type, bool rcu)
{
	...
	if (rcu) {
		 ...
		 * shadow_hook_resolve() rather than #define'd to a bare
		 * name, since this kernel's own struct inode_operations
		 * field is also named get_acl -- see
		 * glue/vendor_kernel_ovl_vfs_compat.h.
		 */
		acl = vns_ovl_vfsc_get_acl(realinode, type);
		ovl_revert_creds(inode->i_sb, old_cred);
	}
	/*
 ...
 * glue/vendor_kernel_ovl_vfs_compat.h) -- its entire body is
