		   ...
		   struct inode *inode, int mask)
{
	...
	err = inode_permission(ovl_mnt_idmap(realpath.mnt), realinode, mask);
	revert_creds(old_cred);
				 ...
				 struct posix_acl *acl, int type)
{
	...
	if (!acl && !upperdentry) {
		...
		ovl_path_lower(dentry, &realpath);
		old_cred = ovl_override_creds(dentry->d_sb);
		real_acl = vfs_get_acl(ovl_mnt_idmap(realpath.mnt), realdentry,
				       acl_name);
		revert_creds(old_cred);
		if (IS_ERR(real_acl)) {
 ...
 * glue/vendor_kernel_ovl_vfs_compat_6_12.h) -- its entire body is
