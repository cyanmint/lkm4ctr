 ...
 * (see glue/vendor_kernel_ovl_vfs_compat.h) -- its entire body is
...
/*
 * [BUILD-COMPAT] non-static (was static "ovl_fs_type"): this vendored copy
 * is deliberately never passed to
 * register_filesystem()/unregister_filesystem() -- doing so with the same
 * name "overlay" the running kernel's own overlayfs (built-in or a
 * separate overlay.ko) may already be using would either collide with
 * -EBUSY, or -- if it "won" the race -- shadow the real one for every
 * caller, not just the ones this module intends to help. Instead,
 * glue/vendor_kernel_overlay.c hooks get_fs_type() and always hands out
 * this struct for the name "overlay", regardless of what the running
 * kernel's own overlayfs support looks like, so "mount -t overlay ..."
 * always resolves to this vendored implementation while lkm4ctr.ko is
 * loaded.
 */
...
int vns_ovl_init(void)
{
	 ...
	 * running kernel (see glue/vendor_kernel_ovl_vfs_compat.h). Must
	 * happen before anything below (or any later ovl_mount()/file
