 ...
 * <linux/fsverity.h>/<linux/namei.h>) are needed so every symbol in the
 * unified glue/vendor_kernel_ovl_vfs_compat.h's VNS_OVL_VFS_COMPAT_LIST()
 * has a visible declaration for typeof() to use, regardless of which .c file
 * in this directory ends up including this header.
 *
 * The unified compat header is included *before* ovl_entry.h because, on
 * pre-6.3 kernels (see the tier layout in that header), it supplies the
 * `struct mnt_idmap` type alias and the ovl_mnt_idmap() accessor that
 * ovl_entry.h's ovl_upper_mnt_idmap() depends on. See
				      ...
				      void *value, size_t size)
{
	...
	err = vfs_getxattr(ovl_mnt_idmap(path->mnt), path->dentry,
			       name, value, size);
...
/*
 * lkm4ctr [BUILD-COMPAT <5.12]: inode_operations::get_acl is
 * (struct inode *, int) here -- it gained the rcu-walk `bool` argument only in
 * 5.11/5.12. Provide a 2-arg thunk so the single 6.12-shaped .get_acl slot can
 * be wired on these kernels (rcu-walk ACL retrieval did not exist yet, so
 * rcu=false is always correct in this era).
 */
static inline struct posix_acl *ovl_get_acl_compat_pre512(struct inode *inode,
							  int type)
{
	return ovl_get_inode_acl(inode, type, false);
}
#endif
#define OVL_GET_INODE_ACL	ovl_get_inode_acl
#define OVL_GET_ACL		ovl_get_acl
#define OVL_SET_ACL		ovl_set_acl
#else
...
/*
 * lkm4ctr [BUILD-COMPAT]: the shape of the ACL inode_operations slots drifts
 * across the supported KMI span, so select the correct field set by version:
 *   >= 6.2  : split .get_inode_acl (inode,type,rcu) + .get_acl (idmap,dentry)
 *             + .set_acl (idmap,dentry).
 *   [5.12,6.2): single .get_acl (inode,type,rcu); overlayfs of this era did not
 *             wire .set_acl (ACLs are still copied up as xattrs) -- so we do not
 *             either, preserving pre-unification behavior.
 *   < 5.12  : single .get_acl (inode,type) via the 2-arg thunk above; no
 *             .set_acl slot wired (same rationale).
 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 2, 0)
#define OVL_IOPS_ACL_FIELDS			\
	.get_inode_acl	= OVL_GET_INODE_ACL,	\
	.get_acl	= OVL_GET_ACL,		\
	.set_acl	= OVL_SET_ACL,
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0)
#define OVL_IOPS_ACL_FIELDS			\
	.get_acl	= OVL_GET_INODE_ACL,
#else
#ifdef CONFIG_FS_POSIX_ACL
#define OVL_IOPS_ACL_FIELDS			\
	.get_acl	= ovl_get_acl_compat_pre512,
#else
#define OVL_IOPS_ACL_FIELDS			\
	.get_acl	= NULL,
#endif
...
#else
/*
 * lkm4ctr [BUILD-COMPAT <6.6]: inode_operations::update_time still carried a
 * `struct timespec64 *` argument before 6.6; the extra arg is unused by the
 * overlayfs implementation (it only touches the upper inode's atime).
 */
