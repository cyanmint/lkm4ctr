 ...
 * glue/shadow_hook.h, duplicated into every module TU that needed them so
 * that no shared .ko was required. Now that vendor_kernel is the only
 * subsystem that hooks syscalls (the former shadow_cgdevices submodule that
 * also hooked syscalls has been removed), that duplication is
 * wasteful and, more importantly, made
 * the recursion guard fragile (see below). The logic now lives here as a
 * single shared implementation; glue/shadow_hook.h is a purely declarative
 ...
 * glue/shadow_hook.h for the full rationale.
 */
 ...
 * shadow_hook_inflight_count() in glue/shadow_hook.h.
 */
 ...
 *
 * See the "rmmod safety" note on struct shadow_hook (glue/shadow_hook.h)
