...
`vendor_kernel_init()`). `Makefile` disables CFI instrumentation
(`CFLAGS_REMOVE_*.o += $(CC_FLAGS_CFI)`) for every object that performs
this kind of call -- `shadow_hijack.c` (the hook engine itself), every
`glue/vendor_kernel_*_compat.c` real-name
...
- `vendor/kernel/nsproxy.o` -- defines `vns_exit_kprobe_pre_handler()`
  (kprobe `pre_handler` on `do_exit()`); makes no unsafe indirect calls of
  its own.
- `vendor/ipc/util.o` -- registers `sysvipc_proc_ops` via
  `proc_create_data()`; makes no unsafe indirect calls of its own.
- `vendor/ipc/shm.o` -- defines `shm_file_operations`/
...
- `vendor/ipc/namespace.o` -- defines `vns_ipcns_operations` (a
...
- `vendor/kernel/utsname.o`, `vendor/kernel/pid_namespace.o`,
  `vendor/kernel/user_namespace.o`,
  `vendor/kernel/cgroup/namespace.o`,
  `vendor/kernel/time/namespace.o`, `vendor/fs/nsfs.o` --
...
The vendored overlayfs sources (`vendor/fs/overlayfs/*.c`) also
...
`glue/lkm4ctr_compat.h`'s `lkm4ctr_lookup_one_len()` calls the
...
- **redroid's Android `init` can abort second-stage boot with `init: Failed to initialize property area` (`InitFatalReboot`) before any app process starts.** Traced against AOSP `android13-release` (`system/core/init/{init,property_service}.cpp`, bionic `libc/system_properties/{system_properties,contexts_serialized,context_node,prop_area}.cpp`): `SecondStageMain()` calls `PropertyInit()` very early, well before cgroup setup or zygote start; `PropertyInit()` -> `__system_property_area_init()` -> `ContextsSerialized::Initialize()` only ever does `mkdir("/dev/__properties__")`, an anonymous `mmap(MAP_PRIVATE|MAP_ANONYMOUS)` for context-node bookkeeping, and per-context `open(..., O_CREAT|O_EXCL|O_NOFOLLOW)`+`fsetxattr(XATTR_NAME_SELINUX)`+`ftruncate`+`mmap(MAP_SHARED)` on plain tmpfs files under `/dev/__properties__`. None of this goes through `unshare`/`clone`/`setns`/`reboot`, or any cgroup/mount-namespace bookkeeping that `vendor_kernel` hooks or vendors -- it is pure userspace tmpfs file I/O inside the container's own already-established mount namespace, so there is no `vendor_kernel` code path to fix here. The failure is consistent with generic redroid/host-kernel SELinux or tmpfs extended-attribute misconfiguration reported independently of this module (e.g. `redroid-doc` issue [#413](https://github.com/remote-android/redroid-doc/issues/413), resolved there by disabling SELinux enforcement on the host). **Workaround**: verify the host kernel actually supports/enables SELinux (or set it permissive/disabled) and that `/dev` inside the container is a real, writable, `xattr`-capable tmpfs (not read-only or missing `CONFIG_TMPFS_XATTR`), before assuming a namespace/cgroup-emulation bug.
- **No vendored cgroup filesystem/controllers, so `systemd`-as-PID-1 containers can fail with `Failed to initialize kmod context: Operation not supported`, then `Unknown filesystem type f2f52010 mounted on /sys/fs/cgroup`.** As described above, `vendor_kernel` only vendors cgroup *namespace* bookkeeping (`CGROUP_NS`) -- it never vendors a `cgroup`/`cgroup2` pseudo-filesystem, controller hierarchy, or `css_set` table the way `fs/overlayfs` is vendored for `mount -t overlay`. If the running kernel cannot actually mount a working `cgroup2` filesystem at `/sys/fs/cgroup` (e.g. `CONFIG_CGROUPS` disabled, or the container runtime's own cgroup mount step silently fails/is skipped), the path handed to the container is left backed by whatever real filesystem underlies it on the device (commonly an F2FS-formatted `/data`/userdata partition, hence `statfs(2)` on `/sys/fs/cgroup` reporting F2FS's `0xf2f52010` magic instead of `CGROUP_SUPER_MAGIC`/`CGROUP2_SUPER_MAGIC`), and `systemd` refuses to boot as PID 1 once it detects this. Building a real, safe cgroup v2 filesystem/controller implementation out-of-tree would be a major new vendored subsystem (on the order of the overlayfs work), not a small fix, and carries the same class of crash risk noted above for the reverted real `NET_NS` attempt, so it is out of scope here. **Workaround**: ensure the target kernel is actually built with working `CONFIG_CGROUPS`/cgroup v2 support and that the container runtime successfully mounts it at `/sys/fs/cgroup` (verify with `mount | grep cgroup` and `stat -f /sys/fs/cgroup` from inside the container/namespace before starting `systemd`), or avoid `systemd`-as-PID-1 container images on kernels that lack this, since no in-kernel-module fix can fabricate real cgroup enforcement.
