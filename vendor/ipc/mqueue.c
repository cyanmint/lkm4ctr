...
/* [BUILD-COMPAT] LKM4CTR ring-buffer logging (see glue/lkm4ctr_log.h);
 * replaces upstream pr_warn/pr_info so all output goes through the module's
 * own log sink, matching the rest of vendor/. */
 ...
 * argument changes); see glue/lkm4ctr_compat.h. */
...
#define mq_timedreceive vns_mq_timedreceive_syscall
#define mq_notify vns_mq_notify_syscall
#define mq_getsetattr vns_mq_getsetattr_syscall
#define mq_open_time32 vns_mq_open_time32
#define mq_notify_time32 vns_mq_notify_time32
				...
				size_t count, loff_t *off)
{
	...
	lkm4ctr_inode_update_ts(file_inode(filp));
	return ret;
}
			...
			struct mq_attr *attr)
{
	...
	return lkm4ctr_inode_permission(d_inode(dentry), acc);
}

static int do_mq_open(const char __user *u_name, int oflag, umode_t mode,
		      struct mq_attr *attr)
{
	struct vfsmount *mnt = vns_current_ipc_ns()->mq_mnt;
	...
	path.dentry = lkm4ctr_lookup_one_len(name->name, root, strlen(name->name));
...
long vns_mq_unlink(const char __user *u_name)
{
	...
	dentry = lkm4ctr_lookup_one_len(name->name, mnt->mnt_root, strlen(name->name));
	if (IS_ERR(dentry)) {
		err = PTR_ERR(dentry);
		goto out_unlock;
	}
	...
	if (!inode) {
		...
		ihold(inode);
		err = lkm4ctr_vfs_unlink(d_inode(dentry->d_parent), dentry,
				 NULL);
	}
	dput(dentry);
		      ...
		      struct mq_attr __user *u_omqstat)
{
	...
	if (ret || !old)
		return ret;
	if (copy_to_user(u_omqstat, old, sizeof(*old)))
		return -EFAULT;
	return 0;
}
