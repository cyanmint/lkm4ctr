...
/*
 * [BUILD-COMPAT] The ->d_real() dentry op changed shape in 6.7: it now takes
 * an `enum d_real_type` (D_REAL_DATA/D_REAL_METADATA) instead of a
 * `const struct inode *`. Provide both forms; the pre-6.7 body is the
 * pre-unification (6.1-era) ovl_d_real().
 */
...
#else /* < 6.7: legacy (const struct inode *) d_real() */
 ...
 * (see glue/vendor_kernel_ovl_vfs_compat_6_12.h) -- its entire body is
...
__nocfi int ovl_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	/*
	 * [BUILD-COMPAT] sb->s_xattr became const-of-const pointers in 6.8;
	 * cast to the field's own type so the assignment is valid on both the
	 * new (no-op cast) and older (silences const-discard) layouts.
	 */
	sb->s_xattr = (typeof(sb->s_xattr))ovl_xattr_handlers(ofs);
	sb->s_fs_info = ofs;
...
int vns_ovl_init(void)
{
	 ...
	 * overlayfs needs but that may not be EXPORT_SYMBOL()'d (or may be
	 * trimmed from the module symbol table) on the running kernel; see
	 * glue/vendor_kernel_ovl_vfs_compat.h.
	 */
	err = vns_ovl_vfs_compat_resolve();
	if (err)
		return err;
